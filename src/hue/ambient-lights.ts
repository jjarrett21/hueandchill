...
import { GroupsAPI, LightsAPI } from "./utility";
import { defaultAmbientLightsRules } from "./ambient-light-rules";
import { setDefaultGroupState } from "./light-group";
...
export async function adapt(languageID: string) {
  ...
  if (hueConfiguration.bridge && hueConfiguration.bridge.isRegistered()) {
    ...
    if (enabled) {
      const rules = Object.assign({}, defaultAmbientLightsRules, userRules);
      ...
      if (rule) {
        ...
        if (lightStates) {
          ...
          for (const lightName of Object.keys(lightStates)) {
            const lightIDs = Object.keys(lights).filter(
              lightID => lights[lightID].name === lightName
            );
          ...
          for (const groupName of Object.keys(groupStates)) {
            const groupIDs = Object.keys(groups).filter(
              groupID => groups[groupID].name === groupName
            );
