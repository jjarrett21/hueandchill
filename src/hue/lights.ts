...
export async function registerNewLights() {
  ...
  vscode.window.showInformationMessage(
    `Found and registered ${lightsSinceLastScan.length} light(s)`
  );
}

export async function searchForLights(
  ...
  for (let second = SEARCH_SECONDS; second > 0; second--) {
    await new Promise(resolve =>
      setTimeout(() => resolve(), SECOND_MILLISECONDS)
    );
    progress.report({
...
export async function turnAllLightsState(state: any, prompt?: boolean) {
  ...
  if (prompt) {
    const inputState = await vscode.window.showInputBox({
      ...
      placeHolder:
        '{"on", "bri", "hue", "sat", "effect", "xt", "ct", "alert", "effect", "transitiontime", "bri_inc", "sat_inc", "hue_inc", "ct_inc", "xy_inc"}'
    });
    try {
      state = JSON.parse(inputState || "");
    } catch {
      vscode.window.showErrorMessage(
        "State is not a valid JSON format. Please try again"
      );
      return;
    }
  }
