...
import { BridgeAPI, ConfigurationAPI } from "./utility";
import { BridgeConfiguration, getHueConfiguration } from "./hue-config";
 ...
 */
export function attemptToRegisterBridge(
  bridgeQuickPickItem: BridgeQuickPickItem
): (progress: vscode.Progress<{ increment: number }>) => Thenable<string> {
  ...
  return async progress => {
    ...
    for (let retryCount = 0; retryCount < MAX_RETRY_COUNT; retryCount++) {
      ...
      await new Promise(resolve =>
        setTimeout(() => resolve(), PAUSE_MILLISECONDS)
      );
...
export async function saveBridgeConfiguration(bridge: BridgeConfiguration) {
  await vscode.workspace
    .getConfiguration()
    .update("hue.bridge", bridge, vscode.ConfigurationTarget.Global);
}
...
export async function registerBridge() {
  ...
  if (hueConfiguration && hueConfiguration.bridge.isRegistered()) {
    ...
    const existingBridge = new BridgeQuickPickItem({
      id: bridge.id,
    ...
    if (
      !overrideSettingDecision ||
      (overrideSettingDecision && !overrideSettingDecision.value)
    ) {
      return;
    }
  }
  ...
  if (!bridges) {
    vscode.window.showWarningMessage(
      "Unable to find Hue Bridge. Make sure that your Hue Bridge is connected to your network your computer is currently on."
    );
    return;
  }
  const bridgeItems = bridges.map(
    (bridge: any) => new BridgeQuickPickItem(bridge)
  );
  const selectedBridge = await vscode.window.showQuickPick<BridgeQuickPickItem>(
    bridgeItems,
    {
      canPickMany: false,
      ignoreFocusOut: false,
  ...
  const registeredUsername = await vscode.window.withProgress(
    {
      location: vscode.ProgressLocation.Notification,
      title:
        "Locate and press the 'link' button on the Hue Bridge you are registering",
      cancellable: true
    },
  ...
  if (!registeredUsername) {
    vscode.window.showErrorMessage(
      "Unable to register Hue Bridge. Please try again"
    );
    return;
  }
  ...
  vscode.window.showInformationMessage(
    `Hue Bridge ${selectedBridge.helpMessage} has successfully been registered.`
  );
